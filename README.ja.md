...
> [!WARNING]
> doc-repoは現在alpha版です。ワークスペースの挙動や編集機能は、今後のリリースで変更される可能性があります。
...
- サイドバーで行をホバーして `+` から新規Markdownドキュメントを作成
- サイドバー行ホバー時のミートボールメニューから、確認付きで文書/フォルダを削除
...
新規作成は、左サイドバーでファイルまたはフォルダ行をホバーして `+` をクリックします。入力はファイル名のみで、入力した名前の末尾に常に `.md` が自動付与され、サイドバー表示は最後の `.md` を外した名前で統一されます。

削除は、左サイドバーで対象行をホバーしてミートボールメニューを開き、**削除** を選択して実行します。フォルダ削除は Policy B に従い、配下に管理対象外またはスコープ外項目がある場合は部分削除を防ぐため拒否されます。
...
ビルド済みのコマンドを起動する場合は、次のコマンドを実行します。
