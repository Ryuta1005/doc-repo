...
以下はすでに `serve` コマンド向けに実装済みである。本 Story ではこれを壊さず、`rootDir` の追加と `generate` コマンドへの適用を完成させることが主目的。
...
| `generate` コマンドへの設定ファイル反映          | **未実装**  | 本 Story の主対象          |
...
**Independent Test**: `rootDir: "./docs"` を含む `doc-repo.config.json` を作成してオプションなしで `generate` と `serve` を実行し、`docs/` 配下のみが対象になることを確認できる。

**Acceptance Scenarios**:

1. **Given** `doc-repo.config.json` に `rootDir: "./docs"` がある状態, **When** `doc-repo generate` を実行する, **Then** 設定ファイルが置かれたディレクトリを基準に相対パスを解決した `docs/` 配下の Markdown が生成される
...
生成者として、`doc-repo.config.json` に `include`/`exclude` を指定しておけば、`generate` と `serve` の両方で同じ収集範囲が使われる状態にしたい。

**Why this priority**: `include`/`exclude` は serve の watch には既に反映されているが、`generate` には未反映のため、生成対象と監視対象が食い違う問題がある。本 Story で統一する。

**Independent Test**: `exclude: ["drafts/**"]` を設定した状態で `generate` を実行し、`drafts/` が出力に含まれないことを確認できる。

**Acceptance Scenarios**:

1. **Given** `exclude: ["drafts/**"]` を設定した状態, **When** `doc-repo generate` を実行する, **Then** `drafts/` 配下のファイルは生成されない
2. **Given** `include: ["specs/**/*.md"]` を設定した状態, **When** `doc-repo generate` を実行する, **Then** `specs/` 配下のみが対象になる
3. **Given** `include` と `exclude` が両方設定されており、あるファイルが両方に一致する状態, **When** `doc-repo generate` を実行する, **Then** そのファイルは exclude が優先され、収集対象にならない
4. **Given** `exclude: ["drafts/**"]` を設定した状態で `serve` を実行中, **When** `drafts/` 配下の Markdown を変更する, **Then** 変更は無視され、不要な再生成が発生しない（watch と generate が一致）
...
**Independent Test**: 設定ファイルなしの状態で `generate` と `serve` を実行し、従来通りの動作が得られることを確認できる。

**Acceptance Scenarios**:

1. **Given** `doc-repo.config.json` が存在しない状態で上位に `.git` がある, **When** `doc-repo generate` を実行する, **Then** `.git` が最初に見つかったディレクトリを rootDir としてサイトが生成される
...
- **FR-010**: システムは `generate` コマンドでも設定ファイルを読み込み、`rootDir`・`include`・`exclude` を反映しなければならない
- **FR-011**: `generate` と `serve` は同じ設定解決結果（rootDir, include, exclude, port）を使用しなければならない
...
- **ResolvedConfig**: ConfigFile・CLI オプション・デフォルト値を優先度順にマージした最終的な設定値。決定済みの `rootDir`・`include`・`exclude`・`port` を保持し、`generate` と `serve` の両方がこれを参照する。

## Success Criteria _(mandatory)_

### Measurable Outcomes

- **SC-001**: `rootDir: "./docs"` を設定した状態でオプションなしの `generate` と `serve` を実行したとき、両コマンドの対象ファイル集合が `docs/` 配下のみで一致する
- **SC-002**: `generate` と `serve` が同一の設定解決結果（rootDir・include・exclude）を使用し、生成対象と監視対象がずれない
...
- `exclude` が `include` に優先されるルールは `generate`・`serve` の両方に共通適用する
