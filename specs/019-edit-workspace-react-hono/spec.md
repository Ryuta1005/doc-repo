...
- 形式: JSON, レスポンス例: `[{ "identifier": "...", "title": "..." }, ...]`
...
- **SC-003**: 文書一覧API、文書取得API、画面切替完了時間を項目別に、同一マシン・同一fixture・同一計測手順で各1回ウォームアップ後に各5回計測し、中央値を記録する。baseline に同等導線が存在する項目は中央値を比較し、悪化率20%未満を原則合格条件とする。019で新規導入したAPIやReact操作のようにbaseline比較できない項目は、baselineをN/Aとしてcurrent測定値と判定理由を記録する。20%以上悪化項目は、原因、利用者影響、019で改善を実施しない理由、許容判断を記録し、明示的承認がある場合のみ合格とする。
...
**Implemented** - Task 019 scope is implemented and verified by automated tests, build checks, architecture documentation, and performance measurement records.

### Follow-up

WYSIWYG エディタ候補の Spike と保存 API は 010 以降で扱う。
