...
- Q: 文書識別子は何を正とするか？ → A: `rootDir` からの正規化相対パスを文書識別子として使用する。APIでの渡し方（パス/クエリ）とURLエンコード方式の詳細は plan で決定する。
...
- Q: FR-013 の回帰テスト要件をどう明確化するか？ → A: React + Honoへの移行前に、Phase 2の主要な利用者体験を回帰テストとして固定し、移行後も同じ期待結果を満たすことを確認する。対象は `doc-repo serve` 起動、左ツリー表示、文書選択と本文表示、相対画像・静的アセット表示、Markdown変更、ファイル追加・削除、ブラウザ自動更新、`doc-repo build`、生成物のオフライン閲覧を含む。
- Q: Success Criteria の性能基準をどう定義するか？ → A: 019の目的は性能改善ではなく体験維持であるため、固定数値は使わない。Markdown変更は既存Phase 2と同等以上の体感で反映され、通常規模fixtureで文書一覧・文書取得・画面切替に著しい性能劣化がないことを基準とする。大量ファイル時は既存実装と同等程度を基準にし、具体的な性能改善は019のスコープ外とする。`build`はファイルサイズ同一ではなく、主要な出力契約とオフライン閲覧方法の維持を基準とする。
...
2. **Given** ツリー構造に新しいファイルが追加される **When** 少し待つ **Then** ツリーが自動更新される
3. **Given** ファイルが削除される **When** 少し待つ **Then** ツリーが自動更新される
...
2. **Given** 文書一覧が取得できた **When** 「文書取得 API」で特定ファイル呼び出し **Then** Markdown HTML と メタデータが返される
...
`doc-repo build` で静的 HTML 出力が生成され、Phase 1 の価値が失われない。

**Why this priority**: MVP の収益価値。読むだけで十分なユーザーには静的出力で対応。

**Independent Test**: `build` コマンドが `.doc-repo` へ静的成果物を生成し、既存と同じ利用方法でオフライン閲覧できることを確認する。

**Acceptance Scenarios**:

1. **Given** `doc-repo build` が実行される **When** 完了 **Then** `.doc-repo` に HTML が生成される
...
- **FR-007**: 文書一覧 API が JSON で文書メタデータを返す（エンドポイント設計は plan で決定）
- **FR-008**: 文書取得 API が `rootDir` からの正規化相対パスを文書識別子として対象文書の HTML を返す（APIでの渡し方とURLエンコード方式の詳細は plan で決定）
- **FR-009**: `doc-repo build` が既存の主要な出力契約とオフライン閲覧方法を維持した静的成果物を生成する
...
- **FR-013**: React + Honoへの移行前に、Phase 2の主要な利用者体験を回帰テストとして固定し、移行後も同じ期待結果を満たすことを確認する。
	- 回帰対象（少なくとも）: `doc-repo serve` の起動、左ツリー表示、文書選択と本文表示、相対画像・静的アセット表示、Markdownの変更、ファイルの追加・削除、ブラウザ自動更新、`doc-repo build`、生成物のオフライン閲覧
...
- API詳細: パラメータ形式（パス/クエリ）とURLエンコード方式は plan で決定
...
- Phase 1 の `build` ロジックを維持し、新しい React 層に影響しない
...
- **SC-002**: Markdown変更は、既存Phase 2と同等以上の体感でブラウザへ反映される
- **SC-003**: 通常規模のfixtureで、文書一覧・文書取得・画面切替に著しい性能劣化がない
- **SC-004**: 移行前に固定・追加した serve・watch・build の回帰テストがすべて通り、移行後も同じ期待結果を満たす
- **SC-005**: 大量ファイル時の性能は既存実装と同等程度を基準とし、具体的な性能改善は019のスコープ外とする
- **SC-006**: React コンポーネント層が core に依存しない（デカップリング）
- **SC-007**: `build` は既存の主要な出力契約とオフライン閲覧方法を維持する
