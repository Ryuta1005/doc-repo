...
**Testing**: Vitest（unit/integration）+ Hono adapter-level HTTP contract tests + 既存 e2e/CLI テストの回帰固定
...
- [x] HTTP 契約検証: pipeline 単体ではなく Hono adapter を通した実 HTTP response で主要な正常系・エラー系を確認
...
**Structure Decision**: 既存の単一 CLI 構成を維持し、`src/core` はフレームワーク非依存のまま維持する。Hono は `src/presentation/http` の HTTP 境界にのみ配置し、処理本体は `src/application` と `src/core` へ委譲する。Viewer は `src/viewer` 配下へ責務名で配置し、core へ直接依存させない。019では既存の `serve` 入口を React Viewer の正規入口とし、React UI・HTTP API・SSE・静的アセットを同一 port / 同一 origin で配信する。静的生成コマンド（`doc-repo [scopePath]`）は既存静的生成フローとオフライン閲覧契約を維持する。
...
8. React Viewer のビルド/同梱/配信方式: 既存 `serve` 入口で同一 port から配信する方式に決定
