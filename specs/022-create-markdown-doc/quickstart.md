...
1. フォルダ行の `+` を押して編集画面へ遷移する
2. ファイル名欄に `sample-folder-child` を入力して保存する
3. 期待結果: 対象フォルダ直下に `sample-folder-child.md` が作成される
4. ファイル行の `+` を押して編集画面へ遷移する
5. ファイル名欄に `sample-file-sibling` を入力して保存する
6. 期待結果: 対象ファイルの親フォルダ直下に `sample-file-sibling.md` が作成される

## 4. 入力ルール確認

1. `+` 押下後の編集画面でファイル名欄に `example` を入力して保存する
2. 期待結果: 内部は `example.md` で作成される
3. `example.md` を入力して保存する
4. 期待結果: 内部は `example.md.md` で作成され、表示名は `example.md`
5. `example.txt` を入力して保存する
6. 期待結果: 内部は `example.txt.md` で作成され、表示名は `example.txt`
7. `child/example` を入力して保存する
8. 期待結果: 作成拒否 + ファイル名のみ入力可の表示

## 5. 表示名ルール確認

1. 作成成功後のサイドバー表示を確認する
2. 期待結果: 表示名は最後の `.md` なし（`example`）
3. 既存 Markdown 文書（作成前から存在する文書）も確認する
4. 期待結果: 既存文書も最後の `.md` なしで表示される
5. ファイルシステム上の実体を確認する
...
- 失敗時は再現手順（入力値、anchor ノード、include/exclude 条件）を残す
- SC-005 は `tests/regression/create-sidebar-latency.md` のテンプレートを使って 10 回計測結果を記録する
